...
public:
    ...
    boost::function<bool (T1,T2)> getServiceCallBackPublisherRobot(bool (*userFunc)(T1,T2,T3*,Robot*),T3* msg, Robot* robot){
        boost::function<bool (T1 req,T2 res)> callBack = [=](T1 req,T2 res) { return userFunc(req,res,msg,robot);};
        return callBack;
    }
    template<typename T1, typename T2>
    boost::function<bool(T1,T2)> getServiceCallBackRobot(bool (*userFunc)(T1,T2,Robot*), Robot* robot){
        boost::function<bool(T1 req,T2 res)> callBack = [=](T1 req,T2 res) {return  userFunc(req,res,robot);};
        return callBack;
    }
